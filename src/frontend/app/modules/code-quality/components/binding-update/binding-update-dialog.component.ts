...
const log = debug('code-quality:binding-update-dialog');
...
export class CodeQualityBindingUpdateDialogComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.codeQualityApi.bindings
      .get(this.data.namespace, this.data.name)
      .subscribe(
        binding => {
          this.model = {
      ...
      .subscribe(
        () => {
          this.message.success(
            this.translate.get('code_quality.update_binding_succ'),
          );
          this.dialogRef.close(true);
        },
        (error: any) => {
          this.notification.error({
            title: this.translate.get('code_quality.update_binding_failed'),
            content: error.error.error || error.error.message,
          });
          this.saving = false;
          this.cdr.markForCheck();
