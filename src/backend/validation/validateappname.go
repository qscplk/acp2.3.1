...
func ValidateAppName(spec *AppNameValiditySpec, client client.Interface) (*AppNameValidity, error) {
	...
	_, err := client.AppsV1beta2().Deployments(spec.Namespace).Get(spec.Name, api.GetOptionsInCache)
	if err != nil {
	...
	_, err = client.CoreV1().Services(spec.Namespace).Get(spec.Name, api.GetOptionsInCache)
	if err != nil {
		if kdErrors.IsNotFoundError(err) || kdErrors.IsForbiddenError(err) {
			isValidService = true
		} else {
