...
type configHook func(map[string]string) map[string]string

var configHooks = []configHook{}

func addHook(hook configHook) {
...
type SettingsManager interface {
	...
	// GetDevopsSettings gets current global settings from config map.
	GetDevopsSettings(client kubernetes.Interface) (s *DevopsSettings)
	// SaveDevopsSettings saves provided global settings in config map.
	SaveDevopsSettings(client kubernetes.Interface, s *DevopsSettings) error
...
func GetDefaultSettingsConfigMap() *corev1.ConfigMap {
	return &corev1.ConfigMap{
		...
		Data: expandConfig(map[string]string{
			GlobalSettingsKey: GetDefaultSettings().Marshal(),
		}),
	}
}

func expandConfig(config map[string]string) map[string]string {
