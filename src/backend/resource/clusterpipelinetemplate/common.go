...
type ClusterPipelineTemplateCell devopsv1alpha1.ClusterPipelineTemplate

func (self ClusterPipelineTemplateCell) GetProperty(name dataselect.PropertyName) dataselect.ComparableValue {
	switch name {
...
func toCells(std []devopsv1alpha1.ClusterPipelineTemplate) []dataselect.DataCell {
	cells := make([]dataselect.DataCell, len(std))
	for i := range std {
		cells[i] = ClusterPipelineTemplateCell(std[i])
	}
	return cells
}

func fromCells(cells []dataselect.DataCell) []devopsv1alpha1.ClusterPipelineTemplate {
	std := make([]devopsv1alpha1.ClusterPipelineTemplate, len(cells))
	for i := range std {
		std[i] = devopsv1alpha1.ClusterPipelineTemplate(cells[i].(ClusterPipelineTemplateCell))
	}
	return std
}
