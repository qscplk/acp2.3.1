...
func NewResourceController(ref meta.OwnerReference, namespace string, client client.Interface) (
	...
	case api.ResourceKindJob:
		job, err := client.BatchV1().Jobs(namespace).Get(ref.Name, api.GetOptionsInCache)
		if err != nil {
			return nil, err
		}
		return JobController(*job), nil
	case api.ResourceKindReplicaSet:
		rs, err := client.AppsV1beta2().ReplicaSets(namespace).Get(ref.Name, api.GetOptionsInCache)
		if err != nil {
			return nil, err
		}
		return ReplicaSetController(*rs), nil
	case api.ResourceKindReplicationController:
		rc, err := client.CoreV1().ReplicationControllers(namespace).Get(ref.Name, api.GetOptionsInCache)
		if err != nil {
			return nil, err
		}
		return ReplicationControllerController(*rc), nil
	case api.ResourceKindDaemonSet:
		ds, err := client.AppsV1beta2().DaemonSets(namespace).Get(ref.Name, api.GetOptionsInCache)
		if err != nil {
			return nil, err
		}
		return DaemonSetController(*ds), nil
	case api.ResourceKindStatefulSet:
		ss, err := client.AppsV1beta2().StatefulSets(namespace).Get(ref.Name, api.GetOptionsInCache)
		if err != nil {
			return nil, err
		}
