...
func GetDaemonSetListChannel(client client.Interface, nsQuery *NamespaceQuery, numReads int) DaemonSetListChannel {
	...
	go func() {
		list, err := client.Apps().DaemonSets(nsQuery.ToRequestParam()).List(api.ListEverything)
...
func GetStatefulSetListChannel(client client.Interface,
	...
	go func() {
		statefulSets, err := client.Apps().StatefulSets(nsQuery.ToRequestParam()).List(api.ListEverything)
...
func GetHorizontalPodAutoscalerListChannel(client client.Interface, nsQuery *NamespaceQuery,
	...
	go func() {
		list, err := client.AutoscalingV2beta1().HorizontalPodAutoscalers(nsQuery.ToRequestParam()).
			List(api.ListEverything)
		for i := 0; i < numReads; i++ {
			channel.List <- list
			channel.Error <- err
