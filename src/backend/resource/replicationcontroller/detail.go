...
import (
	...
	ds "alauda.io/diablo/src/backend/resource/dataselect"
...
func GetReplicationControllerDetail(client k8sClient.Interface, metricClient metricapi.MetricClient,
	...
	replicationController, err := client.CoreV1().ReplicationControllers(namespace).Get(name, api.GetOptionsInCache)
	if err != nil {
		return nil, err
	}
	...
	hpas, err, _ := hpa.GetHorizontalPodAutoscalerListForResource(client, namespace, "ReplicationController", name)
	nonCriticalErrors, criticalError = errors.AppendError(err, nonCriticalErrors)
	if criticalError != nil {
		return nil, criticalError
	}
