...
type bindingCell devopsv1alpha1.ProjectManagementBinding
...
func toCells(std []devopsv1alpha1.ProjectManagementBinding) []dataselect.DataCell {
...
func fromCells(cells []dataselect.DataCell) []devopsv1alpha1.ProjectManagementBinding {
	std := make([]devopsv1alpha1.ProjectManagementBinding, len(cells))
	for i := range std {
		std[i] = devopsv1alpha1.ProjectManagementBinding(cells[i].(bindingCell))
	}
	return std
}
