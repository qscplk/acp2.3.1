...
func setTypeMeta(cm *v1.DaemonSet) {
...
func GetOriginalList(client client.Interface, nsQuery *common.NamespaceQuery, dsQuery *dataselect.DataSelectQuery) (list *v1.DaemonSetList, err error) {
	namespace := nsQuery.ToRequestParam()
	listOptions := common.ConvertToListOptions(dsQuery)
	list, err = client.AppsV1().DaemonSets(namespace).List(listOptions)
...
func DeleteOriginalList(client client.Interface, nsQuery *common.NamespaceQuery, dsQuery *dataselect.DataSelectQuery) (err error) {
	namespace := nsQuery.ToRequestParam()
	listOptions := common.ConvertToListOptions(dsQuery)
	err = client.AppsV1().DaemonSets(namespace).DeleteCollection(nil, listOptions)
	return
}

// GetObjectList returns a slice of runtime.Object
func GetObjectList(list *v1.DaemonSetList) (res []runtime.Object) {
...
func DeleteObject(client client.Interface, obj runtime.Object) (name string, err error) {
	if deploy, ok := obj.(*v1.DaemonSet); ok {
		name = deploy.GetName()
		err = client.AppsV1().DaemonSets(deploy.GetNamespace()).Delete(deploy.GetName(), nil)
	} else {
		err = fmt.Errorf("DaemonSet runtime is not a valid object type: %v", obj)
	}
	return
}
