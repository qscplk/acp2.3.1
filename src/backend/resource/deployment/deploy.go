...
import (
	...
	api1 "alauda.io/diablo/src/backend/settings/api"
...
// share common labels.
func DeployApp(spec *AppDeploymentSpec, client client.Interface, devopsSettings api1.DevopsSettings) error {
	...
	if len(spec.PortMappings) > 0 {
		for _, portMapping := range spec.PortMappings {
			port := api.ContainerPort{
				...
				Name:          strings.ToLower(fmt.Sprintf("%s-%d", portMapping.Protocol, portMapping.TargetPort)),
				Protocol:      portMapping.Protocol,
			}
			containerSpec.Ports = append(containerSpec.Ports, port)
		}
	}
	...
	if len(spec.PortMappings) > 0 {
		...
		// add an ingress
		log.Printf("DefaultDomain: %s; spec.Domain: %s", devopsSettings.DefaultDomain, spec.Domain)
		var host string
		if spec.Domain != "" {
			host = spec.Domain
		} else {
			host = devopsSettings.DefaultDomain
			if host == "" {
				return nil
			}
			host = fmt.Sprintf("%s.%s.%s", service.ObjectMeta.Name, spec.Namespace, host)
		}
		...
		ingressRule := v1beta1.IngressRule{
			...
			IngressRuleValue: v1beta1.IngressRuleValue{
				HTTP: &v1beta1.HTTPIngressRuleValue{
					Paths: make([]v1beta1.HTTPIngressPath, len(spec.PortMappings)),
				},
			},
		}
		for i, portMapping := range spec.PortMappings {
			path := v1beta1.HTTPIngressPath{
				Backend: v1beta1.IngressBackend{
					ServiceName: service.ObjectMeta.Name,
					ServicePort: intstr.IntOrString{
						Type:   0,
						IntVal: portMapping.Port,
					},
				},
			}
			ingressRule.IngressRuleValue.HTTP.Paths[i] = path
		}
		ingress.Spec.Rules = append(ingress.Spec.Rules, ingressRule)
		_, err := client.ExtensionsV1beta1().Ingresses(spec.Namespace).Create(ingress)
		if err != nil {
			return err
		}
	}
