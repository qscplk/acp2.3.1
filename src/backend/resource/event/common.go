...
// GetEventsByUid get events by resource uid.
func GetEventsByUid(client client.Interface, uid string) ([]v1.Event, error) {
	fieldSelector, err := fields.ParseSelector(fmt.Sprintf("involvedObject.uid=%s", uid))
	if err != nil {
		return nil, err
	}

	events, err := client.CoreV1().Events(metaV1.NamespaceAll).List(metaV1.ListOptions{FieldSelector: fieldSelector.String(), ResourceVersion: "0"})
...
func GetNodeEvents(client client.Interface, dsQuery *dataselect.DataSelectQuery, nodeName string) (*common.EventList, error) {
	...
	node, err := mc.Get(nodeName, api.GetOptionsInCache)
	if err != nil {
