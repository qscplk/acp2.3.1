...
type JenkinsCell devopsv1alpha1.Jenkins

func (self JenkinsCell) GetProperty(name dataselect.PropertyName) dataselect.ComparableValue {
	switch name {
...
func toCells(std []devopsv1alpha1.Jenkins) []dataselect.DataCell {
	cells := make([]dataselect.DataCell, len(std))
	for i := range std {
		cells[i] = JenkinsCell(std[i])
	}
	return cells
}

func fromCells(cells []dataselect.DataCell) []devopsv1alpha1.Jenkins {
	std := make([]devopsv1alpha1.Jenkins, len(cells))
	for i := range std {
		std[i] = devopsv1alpha1.Jenkins(cells[i].(JenkinsCell))
	}
	return std
}
