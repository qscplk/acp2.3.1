...
func (self SecretCell) GetProperty(name dataselect.PropertyName) dataselect.ComparableValue {
	...
	case dataselect.SecretTypeProperty:
		return dataselect.StdComparableStringIn(self.Type)
	case dataselect.ProductNameProperty:
		if len(self.ObjectMeta.Annotations) > 0 {
			if self.ObjectMeta.Annotations[common.AnnotationsKeyProduct] != "" {
				return dataselect.StdComparableString(self.ObjectMeta.Annotations[common.AnnotationsKeyProduct])
			}
		}
