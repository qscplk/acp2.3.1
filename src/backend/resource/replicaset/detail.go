...
func GetReplicaSetDetail(client k8sClient.Interface, metricClient metricapi.MetricClient,
	...
	rs, err := client.AppsV1beta2().ReplicaSets(namespace).Get(name, api.GetOptionsInCache)
	if err != nil {
		return nil, err
	}
	...
	hpas, err, _ := hpa.GetHorizontalPodAutoscalerListForResource(client, namespace, "ReplicaSet", name)
	nonCriticalErrors, criticalError = errors.AppendError(err, nonCriticalErrors)
	if criticalError != nil {
		return nil, criticalError
	}
