...
func GetPersistentVolumeClaimList(client kubernetes.Interface, appCoreClient *appCore.ApplicationClient, nsQuery *common.NamespaceQuery,
...
func GetPersistentVolumeClaimListFromChannels(channels *common.ResourceChannels, nsQuery *common.NamespaceQuery,
	dsQuery *dataselect.DataSelectQuery, appCoreClient *appCore.ApplicationClient) (*PersistentVolumeClaimList, error) {
	...
	return toPersistentVolumeClaimList(persistentVolumeClaims.Items, nonCriticalErrors, dsQuery, appCoreClient), nil
}

func toPersistentVolumeClaim(pvc v1.PersistentVolumeClaim, appName string) PersistentVolumeClaim {
...
func toPersistentVolumeClaimList(persistentVolumeClaims []v1.PersistentVolumeClaim, nonCriticalErrors []error,
	dsQuery *dataselect.DataSelectQuery, appCoreClient *appCore.ApplicationClient) *PersistentVolumeClaimList {
	...
	for i := range persistentVolumeClaims {
		setTypeMeta(&persistentVolumeClaims[i])
	}

	appNameList := common.GetAppNameListFromAppcore(appCoreClient, persistentVolumeClaims, filteredTotal)
	if appNameList != nil {
		for i := range persistentVolumeClaims {
			result.Items = append(result.Items, toPersistentVolumeClaim(persistentVolumeClaims[i], appNameList[i]))
		}
	}
