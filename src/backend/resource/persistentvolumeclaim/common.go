...
// The code below allows to perform complex data section on []corev1.PersistentVolumeClaim
...
func GetPodPersistentVolumeClaims(client client.Interface, namespace string, podName string,
	...
	pod, err := client.CoreV1().Pods(namespace).Get(podName, api.GetOptionsInCache)
	if err != nil {
		return nil, err
	}
	...
	if len(claimNames) > 0 {
		...
		podPersistentVolumeClaims := make([]corev1.PersistentVolumeClaim, 0)
		...
		return toPersistentVolumeClaimList(podPersistentVolumeClaims,
			nonCriticalErrors, dsQuery, nil), nil
	}
...
func toCells(std []corev1.PersistentVolumeClaim) []dataselect.DataCell {
...
func fromCells(cells []dataselect.DataCell) []corev1.PersistentVolumeClaim {
	std := make([]corev1.PersistentVolumeClaim, len(cells))
	for i := range std {
		std[i] = corev1.PersistentVolumeClaim(cells[i].(PersistentVolumeClaimCell))
	}
	return std
}
