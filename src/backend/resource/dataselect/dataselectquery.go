...
// GeSimpleLabelQuery get query by 'key == value' in labels
func GeSimpleLabelQuery(key, value string) *DataSelectQuery {
	return NewDataSelectQuery(
		NoPagination,
		NoSort,
		NewFilterQuery([]string{LabelProperty,
			fmt.Sprintf("%s:%s", key, value)}),
		StandardMetrics,
	)
}

// GeSimpleLabelQuery get query by 'key == value'
func GeSimpleFieldQuery(key, value string) *DataSelectQuery {
	return NewDataSelectQuery(
		NoPagination,
		NoSort,
		NewFilterQuery([]string{key, value}),
		StandardMetrics,
	)
}
