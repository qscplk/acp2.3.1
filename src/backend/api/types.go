...
func NewRawObjectMeta(meta ObjectMeta) metaV1.ObjectMeta {
...
func CompleteMeta(meta metaV1.ObjectMeta, old metaV1.ObjectMeta) metaV1.ObjectMeta {
	if old.ResourceVersion != "" {
		meta.ResourceVersion = old.ResourceVersion
	}
	if old.Initializers != nil {
		meta.Initializers = old.Initializers
	}
	if old.Finalizers != nil {
		meta.Finalizers = old.Finalizers
	}
	if old.Generation > 0 {
		meta.Generation = old.Generation
	}
	if old.OwnerReferences != nil {
		meta.OwnerReferences = old.OwnerReferences
	}
	if old.SelfLink != "" {
		meta.SelfLink = old.SelfLink
	}
	if old.GenerateName != "" {
		meta.GenerateName = old.GenerateName
	}
	return meta
}
...
// NewTypeMetaWithVersion creates new type mete for the resource kind.
func NewTypeMetaWithVersion(kind ResourceKind, version string) TypeMeta {
	return TypeMeta{
		Kind:       kind,
		APIVersion: version,
	}
...
const (
	...
	ResourceKindDomain                  = "Domain"
	ResourceKindDomainBinding           = "DomainBinding"
)
...
// ListEverything is a list options used to list all resources without any filtering in cache.
