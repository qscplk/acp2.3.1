...
import (
	...
	syncApi "alauda.io/diablo/src/backend/sync/api"
	"alauda.io/diablo/src/backend/sync/poll"
	"k8s.io/api/core/v1"
...
func (self *secretSynchronizer) Get() runtime.Object {
	...
	if self.secret == nil {
		...
		secret, err := self.client.CoreV1().Secrets(self.namespace).Get(self.name, api.GetOptionsInCache)
		if err != nil {
			return nil
		}
