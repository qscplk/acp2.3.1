...
import (
	...
	setttingsapi "alauda.io/diablo/src/backend/settings/api"
...
func TestClient(t *testing.T) {
	...
	for _, c := range cases {
		...
		manager.SetAuthSettings(&setttingsapi.AuthSettings{Enabled: false})
...
func TestClientCmdConfig(t *testing.T) {
	...
	for _, c := range cases {
		manager := NewClientManager("", "https://localhost:8080")
		manager.SetAuthSettings(&setttingsapi.AuthSettings{Enabled: false})
		cmdCfg, err := manager.ClientCmdConfig(c.request, false)
